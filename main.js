  ...
  async makeRequest(
    ...
    do {
      try {
        ...
        if (error.status === 400) {
          this.log(`Invalid request for ${url}, maybe have new update from server | contact: https://t.me/AllAirdropScript to get new update!`, "error");
...
async function main() {
  ...
  if (!resCheck.endpoint) return console.log(`Cannot find API ID, there may be a connection error, please try again later!`.red);
  ...
  while (true) {
        ...
        workerPromises.push(
          new Promise((resolve) => {
            ...
            worker.on("error", (error) => {
              console.log(`Worker error for the account ${currentIndex}: ${error?.message}`);
              worker.terminate();
              resolve();
            });
            worker.on("exit", (code) => {
              worker.terminate();
              if (code !== 0) {
                errors.push(`Worker error for the account ${currentIndex} exit with code: ${code}`);
              }
              resolve();
            });
          })
    ...
    await sleep(3);
    console.log(`=============${new Date().toLocaleString()} | Complete All Task Wait A |  ${settings.TIME_SLEEP} Minute=============`.magenta);
    showBanner();
